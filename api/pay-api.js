...
//    let stringA = `appid=${opt.appid}&body=${opt.body}&mch_id=${opt.mch_id}&nonce_str=${opt.nonce_str}&notify_url=${opt.notify_url}&out_trade_no=${opt.out_trade_no}&spbill_create_ip=${opt.spbill_create_ip}&total_fee=${opt.total_fee}&trade_type=${opt.trade_type}`
    let stringA =`appid=${unifiedOrderOpt.appid}&body=${unifiedOrderOpt.body}&mch_id=${unifiedOrderOpt.mch_id}&nonce_str=${unifiedOrderOpt.nonce_str}&notify_url=${unifiedOrderOpt.notify_url}&openid=${unifiedOrderOpt.openid}&out_trade_no=${unifiedOrderOpt.out_trade_no}&spbill_create_ip=${unifiedOrderOpt.spbill_create_ip}&total_fee=${unifiedOrderOpt.total_fee}&trade_type=${unifiedOrderOpt.trade_type}`
    
    let stringSignTempA=stringA+"&key="+config.APIKey //key为商户平台设置的密钥key
    
    var md5 = crypto.createHash('md5');
    md5.update(stringSignTempA);
    signA = md5.digest('hex').toUpperCase();
    //sign=MD5(stringSignTemp).toUpperCase() //注：MD5签名方式

    console.log("stringSignTempA: " + stringSignTempA)
    console.log("sign: " + signA)
    

    var bodyData = '<xml>';
    bodyData += '<appid>' + unifiedOrderOpt.appid + '</appid>'; // 小程序ID
    bodyData += '<body>' + unifiedOrderOpt.body + '</body>'; // 商品描述
    bodyData += '<mch_id>' + unifiedOrderOpt.mch_id + '</mch_id>'; // 商户号
    bodyData += '<nonce_str>' + unifiedOrderOpt.nonce_str + '</nonce_str>'; // 随机字符串
    bodyData += '<notify_url>' + unifiedOrderOpt.notify_url + '</notify_url>'; // 支付成功的回调地址
    bodyData += '<openid>' + unifiedOrderOpt.openid + '</openid>'; // 用户标识
    bodyData += '<out_trade_no>' + unifiedOrderOpt.out_trade_no + '</out_trade_no>'; // 商户订单号
    bodyData += '<spbill_create_ip>' + unifiedOrderOpt.spbill_create_ip + '</spbill_create_ip>'; // 终端IP
    bodyData += '<total_fee>' + unifiedOrderOpt.total_fee + '</total_fee>'; // 总金额 单位为分
    bodyData += '<trade_type>JSAPI</trade_type>'; // 交易类型 小程序取值如下：JSAPI
    bodyData += '<sign>' + signA + '</sign>'; 
    bodyData += '</xml>';
    ...
    request({
        ...
        body: bodyData
			...
			console.log('signB'+signB)
                        ...
                        console.log('retrunValue:' + returnValue);
